...
/**
 * 
 * This page displays tutoring and study group announcements to which the currently logged-in user is on the waiting list.
 * Users can view details such as announcement type, title, subject, description, student email, number of enrolled,
 * maximum number of subscribers, hourly rate (if specified), date, time, location and their current position on the list.
 * If the user has not yet subscribed to any waiting list ads, the page still remains viewable with the following message 
 * “Ciao! Al momento non sei iscritto a nessuna lista d'attesa"
 * 
 * 
 */
function PersonalMeeting({ isLoggedIn, handleLogout }) {
  ...
  useEffect(() => {
    ...
    axios.get('http://127.0.0.1:5000/personalWaitingList', {withCredentials: true})
      .then(response => {
  ...
  function handleUnsubscribe(adId) {
    customConfirm("Sicuro di volerti disiscrivere?", (confirmed) => {
      if (confirmed) {
        axios.post('http://127.0.0.1:5000/unsubscribe-waitinglist', { ad_id: adId }, { withCredentials: true })
          .then(() => {
        ...
        <div className="ad-main-info">
          <div className="ad-left">
            <ul>
              ...
              <li><strong>Posizione in lista:</strong> {ad.position}</li>
            </ul>
          </div>
          ...
          {hasAnnouncements ? (
            ...
            <div className="no-announcements">
              ...
              <p className="centered-text">
                <strong> Ciao! </strong> <br /> Al momento non sei iscritto a nessuna lista d&apos;attesa
              </p>
            </div>
          )}
        </div>
