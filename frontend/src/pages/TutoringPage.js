...
/**
 * 
 * This page displays tutoring announcements retrieved from the server. Users can view tutoring details
 * such as the title, subject, description, date, time, location, and the number of enrolled members. Users can
 * also enroll in tutoring ad if they are logged in or are prompted to log in if they are not.
 ...
 */
function TutoringPage({ isLoggedIn, handleLogout }) {
  ...
  useEffect(() => {
    ...
    axios.get('http://127.0.0.1:5000/tutoring', {withCredentials: true})
      .then(response => {
