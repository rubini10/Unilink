    ...
    with create_app().app_context():
        ...
        ads = [
            ...
            Book(title="Libro 2", subject="Analisi",
                 price="15", condition="Nuovo",
                 student_email=user1.email, state="venduto"),
...
def test_bookShop_announcementsVisualization(test_client, setup_database):
    ...
    response = test_client.post('/login',
                                json={'email': "test2@campus.unimib.it",
                                      'password': "Testpassword0!"})
    response = test_client.get('/personal-book')
    ...
    assert len(data) == 3, f"Expected 3 ads, got {len(data)}"

    # Check personal sold book page -there should be 0 books-
    response = test_client.get('/personal-book-sold')
    assert response.status_code == 200

    data = response.get_json()
    # Verify the number of ads if needed
    assert len(data) == 1, f"Expected 1 ads, got {len(data)}"
